...
class Truck extends Vehicle implements AbleToTow {
  ...
  constructor(
    ...
    weight: number,
    topSpeed: number,
    wheels: Wheel[] = [new Wheel(), new Wheel(), new Wheel(), new Wheel()], // Default to 4 new Wheel objects if not provided
    towingCapacity: number,
  ) {
    super(vin, color, make, model, year, weight, topSpeed); // Call the parent constructor
    this.towingCapacity = towingCapacity;

    // Check if the wheels array has 4 elements; if not, create 4 new default Wheel objects
    if (wheels.length !== 4) {
      console.log('Truck wheels must have exactly 4 wheels');
